...
// ════ VOC FORM ════
function changeStep(step) { currentStep=step; for(let i=1;i<=4;i++){document.getElementById('step-content-'+i)?.classList.add('hidden');document.getElementById('node'+i)?.classList.remove('active');} document.getElementById('success-area')?.classList.add('hidden'); document.getElementById('step-content-'+step)?.classList.remove('hidden'); for(let i=1;i<=step;i++)document.getElementById('node'+i)?.classList.add('active'); if(step!==4)resetTurnstile(); }
